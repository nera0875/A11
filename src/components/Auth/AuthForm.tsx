...
export const AuthForm: React.FC<AuthFormProps> = ({ onSuccess }) => {
  ...
  const { signIn, signUp, signInTest, isLoading, error } = useAppStore()

  const handleTestLogin = async () => {
    console.log('🧪 [TEST-LOGIN] Tentative de connexion test...')
    
    try {
      await signInTest()
      console.log('🧪 [TEST-LOGIN] Connexion test réussie!')
      onSuccess?.()
    } catch (error) {
      console.error('🧪 [TEST-LOGIN] Erreur lors de la connexion test:', error)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('🔐 [AUTH-FORM] Début de soumission du formulaire:', {
      isSignUp,
      email,
    ...
    try {
      if (isSignUp) {
        console.log('📝 [AUTH-FORM] Tentative d\'inscription pour:', email)
        const result = await signUp(email, password)
        console.log('📝 [AUTH-FORM] Résultat inscription:', {
        ...
        if (result?.requiresEmailConfirmation) {
          ...
          console.log('✅ [AUTH-FORM] Email de confirmation envoyé')
        } else {
          console.log('✅ [AUTH-FORM] Inscription réussie - redirection')
          onSuccess?.()
        }
      } else {
        console.log('🔑 [AUTH-FORM] Tentative de connexion pour:', email)
        await signIn(email, password)
        console.log('✅ [AUTH-FORM] Connexion réussie - redirection')
        onSuccess?.()
      }
    } catch (err) {
      console.error('❌ [AUTH-FORM] Erreur lors de la soumission:', err)
