...
export const TerminalViewer: React.FC<TerminalViewerProps> = ({
  ...
  }, [execution?.output, execution?.error, streamingOutput])

  // Parse streaming output to separate stdout and stderr
  const parseStreamingOutput = (output: string) => {
    ...
    lines.forEach(line => {
      if (line.startsWith('[STDERR]')) {
        stderr.push(line.replace('[STDERR]', ''))
      } else if (line.startsWith('[STDOUT]')) {
        stdout.push(line.replace('[STDOUT]', ''))
      } else if (line.trim()) {
        stdout.push(line)
      }
    })
  ...
  const { stdout: streamStdout, stderr: streamStderr } = parseStreamingOutput(streamingOutput)
            ...
            {execution ? (
              <div className="h-full flex flex-col">
                ...
                >
                  ...
                  {isExecuting && streamingOutput && (
                    <div className="mb-4">
                      {/* Streaming Standard Output */}
                      {streamStdout && (
                        <div className="text-gray-100 whitespace-pre-wrap break-words mb-2">
                          <div className="text-green-300 text-xs mb-1 opacity-75">STDOUT:</div>
                          {streamStdout}
                        </div>
                      )}
                      
                      {/* Streaming Error Output */}
                      {streamStderr && (
                        <div className="text-red-400 whitespace-pre-wrap break-words mb-2">
                          <div className="text-red-300 text-xs mb-1 opacity-75">STDERR:</div>
                          {streamStderr}
                        </div>
                      )}
                      ...
                      <div className="flex items-center space-x-2 text-green-400 mt-2">
                        ...
                        <span className="text-xs opacity-75">Streaming en temps réel...</span>
                      </div>
                    </div>
                  )}

                  {/* Historical Output - From completed execution */}
                  {execution && (
                    <>
                      {/* Standard Output */}
                      {execution.output && (
                      ...
                      {/* Exit Code */}
                      {execution.exit_code !== null && !isExecuting && (
                        <div className={`mt-4 p-2 rounded border-l-4 text-sm ${
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <span>Code de sortie: {execution.exit_code}</span>
                            <span className="text-xs opacity-75">
                              {execution.exit_code === 0 ? '(Succès)' : '(Erreur)'}
                            </span>
                          </div>
                        </div>
                      )}
                  ...
                  {/* Completion Status for Streaming */}
                  {!isExecuting && !execution && streamingOutput && (
                    <div className="mt-4 p-2 rounded border-l-4 text-sm text-green-400 border-green-400 bg-green-900/20">
                      <div className="flex items-center space-x-2">
                        <div className="w-2 h-2 bg-green-400 rounded-full"></div>
                        <span>Exécution terminée</span>
                      </div>
                    </div>
                  )}
                </div>
